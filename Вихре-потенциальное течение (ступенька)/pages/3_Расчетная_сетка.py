...
if menu == "Описание сетки":
    ...
    **Генерация сетки в Gmsh**
    ...
    1. Определение геометрических точек и отрезков
    ...
    **Физические группы**
    ...
    if os.path.exists(stats_file):
        ...
        for s in stats:
            if s['level'] == level_select:
                png_file = s['png']
                nodes = s['nodes']
                elements = s['elements']

                if os.path.exists(png_file):
                    st.image(png_file, caption=f"Сетка {level_select} (узлов: {nodes}, элементов: {elements})", use_container_width=True)
                else:
                    st.warning(f"Изображение не найдено: {png_file}")
                break
    else:
        st.warning(f"Файл статистики не найден: {stats_file}")
        st.info("Запустите генератор сеток и визуализацию: `python generate_meshes.py`")
